...
#@RESNEST_MODELS_REGISTRY.register()
def resnest50_3D(pretrained=False, ipt_dim=3, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('/remote-home/share/21-yuanruntian-21210240410/da/models/resnest50-528c19ca.pth'))
        # model.load_state_dict(torch.hub.load_state_dict_from_url(
        #     resnest_model_urls['resnest50'], progress=True, check_hash=True))
        # print('after loading')
        # print(model.conv1[0].weight[0])
    ...
    model_3d = ResNet3D(model, Bottleneck3D, [3, 4, 6, 3], ipt_dim=ipt_dim,
                   radix=2, groups=1, bottleneck_width=64,
                   deep_stem=True, stem_width=32, avg_down=True,
                   avd=True, avd_first=False, **kwargs)
    return model_3d
